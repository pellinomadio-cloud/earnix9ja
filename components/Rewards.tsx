  ...
  return (
    <div className="bg-black min-h-screen animate-in fade-in duration-500">
      <div className="px-4 py-4 space-y-6">
        ...
        {/* Header with Gold Back Button */}
        <div className="flex items-center pt-2">
            <button 
                onClick={onBack} 
                className="bg-gold hover:bg-gold-dark text-black px-6 py-2 rounded-full flex items-center text-sm font-bold shadow-md transition-colors"
            >
                 <Icons.ArrowLeft size={18} className="mr-2" strokeWidth={3} /> Back
            </button>
            <h1 className="ml-4 text-2xl font-bold text-gold text-glow-gold">Earnix9ja</h1>
        </div>

        {/* Progress Card */}
        <div className="bg-gray-900 rounded-3xl p-6 shadow-gold border border-gold/20">
            ...
            <div className="w-full bg-black rounded-full h-4 border border-gray-800 overflow-hidden">
                <div 
                    className="bg-gold h-4 rounded-full transition-all duration-500 shadow-gold-lg animate-gold-pulse"
                    style={{ width: `${progressPercentage}%` }}
                ></div>
            </div>
        </div>

        {/* Gold Timer/Action Card */}
        <div className="bg-gold rounded-3xl p-8 text-center text-black shadow-gold-lg relative overflow-hidden group">
            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/30 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000"></div>
            <div className="relative z-10 flex flex-col items-center justify-center">
                <div className="mb-4 bg-black rounded-full p-2 shadow-gold">
                     <Icons.Clock size={32} className="text-gold" fill="currentColor" />
                </div>
                ...
                {!canClaim ? (
                    ...
                    <div className="mb-2 w-full">
                        <p className="text-black/70 text-sm mb-2 font-medium">Your reward is ready!</p>
                         <button 
                            onClick={onClaim}
                            className="w-full px-8 py-4 bg-black text-gold font-bold text-xl rounded-xl transition-all shadow-gold-lg animate-gold-glow-button flex items-center justify-center"
                        >
                            Ready to Claim! 🎉
                        </button>
                    </div>
            ...
            <style>{`
              @keyframes gold-glow-button {
                0% { box-shadow: 0 0 5px rgba(255, 215, 0, 0.4); filter: brightness(1); }
                50% { box-shadow: 0 0 25px rgba(255, 215, 0, 0.9); filter: brightness(1.4); }
                100% { box-shadow: 0 0 5px rgba(255, 215, 0, 0.4); filter: brightness(1); }
              }
                ...
                return (
                    <div 
                        ...
                        className={`
                            ...
                            ${isClaimed 
                                ? 'bg-gray-900 border-gold shadow-[0_0_5px_rgba(255,215,0,0.1)]' 
                                : isCurrent && canClaim
                                    ? 'bg-gray-900 border-gold shadow-lg transform scale-105 z-10 ring-1 ring-gold/50' 
                                    : 'bg-gray-900/50 border-gray-800'
                            }
                        `}
                    >
                        <span className="text-[10px] font-medium text-gray-500 mb-1">Day {day}</span>
                        <span className={`text-[9px] font-black mb-2 ${isClaimed || isCurrent ? 'text-gold' : 'text-gray-600'}`}>
                            ₦100,000
                        </span>
                        {isClaimed && (
                            <div className="flex items-center text-[8px] text-gold font-bold bg-gold/10 px-1.5 py-0.5 rounded-full">
                                <span className="mr-0.5">✓</span> Claimed
                            </div>
                        )}
                         {isCurrent && !isClaimed && (
                            <span className={`text-[8px] ${canClaim ? 'bg-gold text-black animate-pulse' : 'bg-gray-800 text-gray-500'} px-2 py-0.5 rounded-full font-bold`}>

...
const InviteEarn: React.FC<InviteEarnProps> = ({ onReward, onBack }) => {
  ...
  const handleTaskClick = (index: number) => {
    ...
    const text = encodeURIComponent("Join Earnix9ja today and get ₦10,000 welcome bonus! Join here: https://earnix.top");
    ...
    <div className="px-4 py-6 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-24">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-sm text-gray-500 max-w-xs mx-auto">
            Complete 5 sharing tasks to instantly receive <span className="font-bold text-gold">₦50,000</span> in your wallet.
        </p>
      </div>

      {/* Progress Card */}
      <div className="bg-gradient-to-r from-gold to-gold-dark rounded-2xl p-6 text-black shadow-lg relative overflow-hidden">
      ...
      {/* Cooldown Timer */}
      {cooldown > 0 && (
         <div className="bg-gold/5 border border-gold/20 p-4 rounded-xl flex items-center justify-center space-x-3 animate-pulse">
            <Icons.Clock className="text-gold" />
                ...
                <button
                    ...
                    className={`w-full p-4 rounded-xl border-2 flex items-center justify-between transition-all duration-200 ${
                        ...
                        : isCurrent && cooldown === 0
                            ? 'bg-gray-900 border-gold shadow-md transform scale-[1.02]' 
                ...
                >
                    <div className="flex items-center space-x-4">
                        ...
                        <div className="text-left">
                            <h4 className={`font-bold text-sm ${isCompleted ? 'text-gold' : 'text-white'}`}>
                    ...
                    <div>
                         {isCurrent && isLoading ? (
                            <Icons.Sync className="animate-spin text-gold" size={20} />
                         ) : isCompleted ? (
                             <span className="text-xs font-bold text-gold">Done</span>
                         ) : (
                             <Icons.MessageCircle className={`${isCurrent && cooldown === 0 ? 'text-gold' : 'text-gray-700'}`} size={24} />
                         )}
                    </div>
                </button>
            );

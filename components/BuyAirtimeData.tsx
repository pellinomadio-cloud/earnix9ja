  ...
  if (step === 'success') {
    return (
        <div className="px-4 py-8 flex flex-col items-center justify-center text-center space-y-6 animate-in zoom-in duration-300">
          ...
          <div>
              <h2 className="text-2xl font-bold text-white mb-1">Purchase Successful!</h2>
  ...
  return (
    <div className="px-4 py-6 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="text-center">
        <div className={`inline-flex items-center justify-center p-3 rounded-full mb-2 bg-gold/10 text-gold`}>
            {type === 'airtime' ? <Icons.Airtime size={32} /> : <Icons.Data size={32} />}
        </div>
        <h2 className="text-xl font-bold text-white capitalize">Buy {type}</h2>
      </div>
      ...
      <form onSubmit={handlePurchase} className="space-y-6">
                ...
                {networks.map((net) => (
                    <div 
                        ...
                        className={`
                            ...
                            ${network === net.id ? 'border-gold bg-gold/5' : 'border-gray-800 bg-gray-900'}
                        `}
                    >
        ...
        <div>
            ...
            <div className="relative">
                ...
                <input
                    ...
                    className="w-full pl-10 p-3 bg-gray-900 border border-gray-800 rounded-xl text-white focus:ring-2 focus:ring-gold outline-none font-mono text-lg tracking-wide"
                />
            </div>
        </div>
        ...
        {type === 'airtime' ? (
            <div>
                ...
                <input
                    ...
                    placeholder="100"
                    className="w-full p-3 bg-gray-900 border border-gray-800 rounded-xl text-white font-bold text-lg focus:ring-2 focus:ring-gold outline-none"
                />
                <div className="flex gap-2 mt-2 overflow-x-auto no-scrollbar pb-1">
                    {[100, 200, 500, 1000, 2000].map(amt => (
                        <button
                            ...
                            className="px-3 py-1 bg-gray-800 text-gray-300 rounded-full text-xs font-bold whitespace-nowrap hover:bg-gold/10 hover:text-gold"
                        >
                            ₦{amt}
                        </button>
                    ))}
                ...
                <div className="space-y-2">
                    {dataPlans.map(plan => (
                        <div 
                            ...
                            className={`p-3 rounded-xl border flex justify-between items-center cursor-pointer transition-all ${selectedPlan === plan.id ? 'border-gold bg-gold/5' : 'border-gray-800 bg-gray-900'}`}
                        >
                            <span className="font-bold text-white text-sm">{plan.name}</span>
                            <span className="font-bold text-gold">₦{plan.price.toLocaleString()}</span>
                        </div>
                    ))}
                </div>
            </div>
        ...
        <button
            ...
            className="w-full py-4 bg-gold hover:bg-gold-dark disabled:bg-gray-800 text-black font-bold rounded-full shadow-lg transition-all mt-4 flex items-center justify-center space-x-2"
        >
            {isLoading ? (
